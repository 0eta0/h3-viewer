...
var app = new Vue({
  ...
  methods: {
    ...
    findH3: function () {
      const h3indexes = this.searchH3Ids.split(",");
      if (!h3indexes.every((e) => h3.isValidCell(e))) return;

      h3indexes.forEach((e) => {
        const h3Boundary = h3.cellToBoundary(e);
      ...
      const newZoom =
        H3_RES_TO_ZOOM_CORRESPONDENCE[h3.getResolution(h3indexes[0])];
      map.setZoom(newZoom);
    },
  },
